...
#' @param lambda scalar of weights of the UNlabelled observations as per the knownStates vector. Default: 1.0 (equal weighting of labelled and unlabelled examples)
#' a value less than 1.0 downweights unlabelled observations, and a value more than 1.0 upweights unlabelled observagtions.
...
fitHMM.momentuHMMData <- function(data,nbStates,dist,
                   ...
                   mvnCoords=NULL,stateNames=NULL,knownStates=NULL,lambda=1.0,fixPar=NULL,retryFits=0,retrySD=NULL,optMethod="nlm",control=list(),prior=NULL,modelName=NULL, ...)
{
  ...
  if(fit) {
    ...
    while(fitCount<=retryFits){
      ...
      if(all(distnames %in% c("step","angle")) & all(unlist(dist) %in% moveHMMdists) & mHind & (lambda == 1.0)){
        ...
        if(!length(optPar)){
          ...
          curmod$minimum <- nLogLike(optPar,nbStates,newformula,p$bounds,p$parSize,data,inputs$dist,covs,
                                    ...
                                    stationary,fullDM,DMind,p$Bndind,knownStates,lambda,unlist(fixParIndex$fixPar),fixParIndex$wparIndex,
  ...
  mh <- list(data=data,mle=mle,mod=mod,conditions=conditions,rawCovs=rawCovs,stateNames=stateNames,knownStates=knownStates,lambda=lambda,covsDelta=covsDelta,prior=prior,modelName=modelName,reCovs=recovsCol,g0covs=g0covsCol,covsPi=covsPi)
  ...
  mh <- list(data=data,mle=mle,CIreal=CIreal,CIbeta=CIbeta,mod=mod,conditions=conditions,rawCovs=rawCovs,stateNames=stateNames,knownStates=knownStates,lambda=lambda,covsDelta=covsDelta,prior=prior,modelName=modelName,reCovs=recovsCol,g0covs=g0covsCol,covsPi=covsPi)
...
fitHMM.momentuHierHMMData <- function(data,hierStates,hierDist,
                       ...
                       mvnCoords=NULL,knownStates=NULL,lambda=1.0,fixPar=NULL,retryFits=0,retrySD=NULL,optMethod="nlm",control=list(),prior=NULL,modelName=NULL, ...)
{
  ...
  hfit <- fitHMM.momentuHMMData(momentuHMMData(data),inputHierHMM$nbStates,inputHierHMM$dist,Par0,inputHierHMM$beta,inputHierHMM$delta,
                ...
                mvnCoords,inputHierHMM$stateNames,knownStates,lambda=1.0,inputHierHMM$fixPar,retryFits,retrySD,optMethod,control,prior,modelName)
